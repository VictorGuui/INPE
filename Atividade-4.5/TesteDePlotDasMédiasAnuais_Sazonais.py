...
def create_plot(ax,nrow, ncol, data, tlon, season = ''):
    ...
    print(f'Create_plot normal {data.coords.keys()}')
...
def create_plot_MediaAnual(ax,nrow, ncol, data, tlon, season = ''):
    ...
    cnplotMediaAnual = ax.contourf(data.longitude, data.latitude, data.sel(season=season), levels= levels, cmap='YlGnBu', extend='max') 
    ax.text(tlon, -55, season, bbox=dict(facecolor='white', alpha=0.7))
    print(f'Create_plot Anual {data.coords.keys()}')
...
def create_plot_MediaSazonal(ax,nrow, ncol, data, tlon, season = ''):
    ...
    cnplotMediaSazonal = ax.contourf(data.longitude, data.latitude, data.sel(season=season), levels= levels, cmap='YlGnBu', extend='max') 
    ax.text(tlon, -55, season, bbox=dict(facecolor='white', alpha=0.7))
    print(f'Create_plot Sazonal {data.coords.keys()}')
...
seasons = ['DJF', 'MAM', 'JJA', 'SON']
seasonsSazonal = ['DJF', 'MAM', 'JJA', 'SON']
seasonsAnual = ['JJA',]
...
for i, season in enumerate(seasons):
    ...
    create_plot(plt.subplot(gs[row, col], projection=ccrs.PlateCarree()), row, col, sclimMediaAnual, -44, seasons[i])
    create_plot_MediaAnual(plt.subplot(gs[row, col], projection=ccrs.PlateCarree()), row, col, sclimMediaAnual, -44, seasonsAnual[i])
    create_plot_MediaSazonal(plt.subplot(gs[row, col], projection=ccrs.PlateCarree()), row, col, sclimMediaSazonal, -44, seasonsSazonal[i])
